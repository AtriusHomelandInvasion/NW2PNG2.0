...
public class NW2PNGHelper implements Runnable {
    ...
    ArrayList<RenderObj> render_objects = new ArrayList<RenderObj>();
        ...
        try {
            ...
            while (level_file_line != null) {
                ...
                if (level_file_line.startsWith("BOARD ")) {
                    ...
                    boolean foundshowcharacter = false, foundsetimgpart = false, foundignorerender = false;
                    ...
                    for (int j = 0; j < 300; j++) {
                        ...
                        if (npc_imgpart.startsWith("showcharacter") || npc_imgpart.startsWith("this.showcharacter")) {
                            ...
                            if (npc_imgpart.indexOf(",") > -1) {
                                String[] tokens = npc_imgpart.split(",");
                        ...
                        } else if (npc_imgpart.startsWith("setimgpart") || npc_imgpart.startsWith("this.setimgpart")) {
                            // If 'setimgpart' is found, append its values to the render data
                            npc_imgpart = npc_imgpart.replaceAll("\"", "");
                            npc_imgpart = npc_imgpart.replace("this.", "");
                            npc_imgpart = npc_imgpart.replace("(", " ");
                            ...
                            if (!foundignorerender) {
                                level_file.add("NPC " + imgname + " " + NPCx + " " + NPCy + imgpartvalues);
                            }
                            ...
                            if ((attrs[0] != null || foundshowcharacter) && !foundignorerender && getRenderChars()) {
                                if (attrs[0] == null) {
                                    attrs[0] = "idle.gani";
                                }
                    ...
                    if (foundsetimgpart == false && !level_file_line.startsWith("NPC -") && !foundignorerender) {
                        level_file.add(level_file_line + " 0 0 -1 -1");
                    }
                    level_reader.reset();
                }
            ...
            for (String[][] gani : ganis) {
                if (gani[0][12] != null && getRenderChars() == false) {
                    continue;
                }
                ...
                int[] arr = {NPCx, NPCy, NPCw, NPCh};
                render_objects.add(new RenderObj(gani_render, arr, level_name, 2));
            ...
            for (String[] npc : level_npcs) {
                ...
                if (npc[0] == null || npc[0].toLowerCase().contains("light")) {
                    continue;
                }
                ...
                try {
                    if (getImageLocation(npc[0]) == null) {
                        npc[0] = null;
                        continue;
                    }
                    ...
                    if (npc_image == null) {
                        ...
                        int[] arr = {render_x, render_y, render_x + render_w, render_y + render_h,
                            image_dx, image_dy, image_dx + image_dw, image_dy + image_dh};
                        render_objects.add(new RenderObj(npc_image, arr, level_name, 1));
                    }
                } catch (IOException e) {
                    writeLog(e);
            ...
            if (renderinggmap == false || getSplit() == true) {
                render_objects = sortRender(render_objects);
                for (RenderObj i : render_objects) {
            ...
            if (renderinggmap == false) {
                getListener().sendMessage("Rendering and saving the image...");
            }
    ...
    private BufferedImage renderGmap(File source) {
        ...
        try {
            ...
            if (getRenderNPCs() == true) {
                ...
                for (RenderObj obj : render_objects) {
                    int lvlcoord = 0;
                    for (int i = 0; i < levels.length; i++) {
                        if (obj.getLevel().equals(levels[i])) {
                            lvlcoord = i;
                            break;
                        } else {
                            continue;
                    ...
                    int draw_x = (lvlcoord % gmap_width) * intDimension;
                    int draw_y = (int) (lvlcoord / gmap_width) * intDimension;
    ...
    private void CheckFILENAMECACHE() {
        ...
        if (!filenamecacheDir.endsWith("\\")) {
            filenamecacheDir = filenamecacheDir + "\\";
        }
        ...
        if (filecheck.exists() == false) {
            ...
            if (filecheck2.exists() == false) {
                getListener().sendMessage("Error: Failed to find FILENAMECACHE.txt on fallback search.");
            }
        } else {
        }
    }
    ...
    private BufferedImage getGani(String[] attr) {
        ...
        try {
            ...
            for (int i = 0; i < 300; i++) {
                ...
                if (sprite_scan == null || !sprite_scan.startsWith("SPRITE")) {
                    break;
                }
            ...
            for (int i[] : rendersprites) {
                ...
                if (sprite_Y < sprite_minY) {
                    sprite_minY = sprite_Y;
                }
            }

            if (sprite_minX < 0) {
                ganiOffsetx = sprite_minX;
            }
            if (sprite_minY < 0) {
                ganiOffsety = sprite_minY;
            }
